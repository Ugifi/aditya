...
function SPMotorSection({ num, setNum, amt, setAmt, chips, onSubmit, openClose, submitting }) {
  ...
  return <>
    <div className="bf-fg">
      ...
      {parsedDigits.length > 0 && parsedDigits.length < 3 && (<div className="bf-desc-box" style={{ background: 'rgba(255,165,0,0.1)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)', marginTop: 10 }}>⚠️ Kam se kam <strong>3 unique digits</strong> chahiye</div>)}
    </div>
...
function RedJodiSection({ num, setNum, amt, setAmt, chips, onSubmit, openClose, submitting }) {
  ...
  return <>
    ...
    {redType === '' && (<div className="bf-desc-box" style={{ background: 'rgba(255,165,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)' }}>⬆️ Pehle <strong>Half Red Jodi</strong> ya <strong>Full Red Jodi</strong> select karo</div>)}
    {redType !== '' && (
      <div className="bf-fg">
  ...
  return <>
    <div className="bf-fg">
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
        {familyKeys.map(fk => (
          <div key={fk} onClick={() => handleFamilySelect(fk)} className={`bf-chip${selectedFamily === fk ? ' active' : ''}`} style={{ padding: '10px 14px', cursor: 'pointer', fontWeight: 700, fontSize: 13, minWidth: 70, textAlign: 'center', background: fk === 'half_red' ? (selectedFamily === fk ? '#e53935' : 'rgba(229,57,53,0.1)') : fk === 'full_red' ? (selectedFamily === fk ? '#b71c1c' : 'rgba(183,28,28,0.1)') : undefined, color: (fk === 'half_red' || fk === 'full_red') ? (selectedFamily === fk ? '#fff' : '#e53935') : undefined, borderColor: (fk === 'half_red' || fk === 'full_red') ? '#e53935' : undefined }}>
            {JODI_FAMILY_LABELS[fk]}
          </div>
        ))}
      </div>
    </div>
    {!selectedFamily && (<div className="bf-desc-box" style={{ background: 'rgba(255,165,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)' }}>⬆️ Upar se koi ek <strong>Family</strong> select karo</div>)}
    {selectedFamily && <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, gap: 8 }}>
        <div className="bf-desc-box" style={{ background: 'rgba(255,107,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)', margin: 0, flex: 1 }}>✅ <strong>{JODI_FAMILY_LABELS[selectedFamily]}</strong> — <strong>{selectedJodis.length}/{familyJodis.length}</strong> Jodis selected</div>
        <div onClick={toggleAll} style={{ padding: '8px 12px', background: selectedJodis.length === familyJodis.length ? '#e53935' : '#FF6B00', color: '#fff', borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>{selectedJodis.length === familyJodis.length ? '✕ Clear' : '✓ All'}</div>
      </div>
      <div className="bf-fg">
      ...
      {selectedJodis.length === 0 && (<div className="bf-desc-box" style={{ background: 'rgba(255,165,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)' }}>⚠️ Kam se kam <strong>1 jodi</strong> select karo</div>)}
...
function FamilyPanaSection({ num, setNum, amt, setAmt, chips, openClose, onSubmit, submitting }) {
  ...
  return <>
    ...
    {num.length === 3 && !foundFamily && (<div className="bf-desc-box" style={{ background: 'rgba(255,165,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)' }}>⚠️ Yeh pana kisi family mein nahi mila. Sahi pana enter karo.</div>)}
    {foundFamily && <>
      <div className="bf-desc-box" style={{ background: 'rgba(255,107,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)' }}>✅ <strong>{foundFamily} Family</strong> — Total <strong>{totalLines} Panas</strong> cover hongi</div>
      <div className="bf-fg">
      ...
      {Number(amt) >= 10 && (<button className="bf-place-btn" onClick={handleSubmit} disabled={submitting} style={{ opacity: submitting ? 0.6 : 1, cursor: submitting ? 'not-allowed' : 'pointer' }}>{submitting ? '⏳ Placing...' : `🎯 Place Family Bid — ₹${totalAmt.toLocaleString()}`}</button>)}
    </>}
  </>;
}
...
{(id === 'single_digit' || id === 'single_digit_close') && <><div className="bf-fg"><label className="bf-label">Pick a Digit (0–9)</label><NumGrid selected={num} onSelect={v => { setNum(v); setActiveN(Number(v)); }} /></div><AmtInput amt={amt} setAmt={setAmt} chips={chips}/><WinInfo/><PlaceBtn/></>}        {id === 'single_digit_bulk' && <><div className="bf-fg"><label className="bf-label">Pick Digits</label><NumGrid selected={num} onSelect={setNum} /></div><AmtInput amt={amt} setAmt={setAmt} chips={chips} label="Bid Amount (Min ₹10)"/><AddBtn label="+ Add Digit"/><BulkTable/>{bets.length > 0 && <PlaceAllBtn/>}</>}
        ...
        {(id === 'full_sangam' || id === 'full_sangam_bulk') && <>
          <div className="bf-fg">
            <label className="bf-label" style={{display:'flex', justifyContent:'space-between', width:'100%'}}>
              <span>Open Pana</span>
              {num2 && <span style={{fontSize:12, color:'#FF6B00', fontWeight:'bold'}}>Close Selected: {num2} ✓</span>}
            </label>
          ...
          <div className="bf-fg">
            <label className="bf-label" style={{display:'flex', justifyContent:'space-between', width:'100%'}}>
              ...
              {num && <span style={{fontSize:12, color:'#FF6B00', fontWeight:'bold'}}>Open Selected: {num} ✓</span>}
            </label>
            <PanaGrid panas={PANA_210} selected={num2} onSelect={setNum2} />
          </div>
          {(!num || !num2) && <div className="bf-desc-box" style={{background:'rgba(255,165,0,0.08)', color:'#E65C00', border:'1px solid rgba(230,92,0,0.3)'}}>Please select both <strong>Open Pana</strong> and <strong>Close Pana</strong>.</div>}
          <AmtInput amt={amt} setAmt={setAmt} chips={chips}/>
          <AddBtn label="+ Add to List"/>
          <BulkTable/>
          {bets.length > 0 && <PlaceAllBtn/>}
        ...
        {id === 'dp_motor' && (() => {
          ...
          return <>
            <div className="bf-fg">
              ...
              {parsedDigits.length > 0 && parsedDigits.length < 3 && (<div className="bf-desc-box" style={{ background: 'rgba(255,165,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)', marginTop: 10 }}>⚠️ Kam se kam <strong>3 unique digits</strong> chahiye</div>)}
            </div>
            <AmtInput amt={amt} setAmt={setAmt} chips={chips} label="💰 Amount per Pana (Min ₹10)" />
        ...
        {id === 'sp_dp_tp' && (() => {
          ...
          return <>
            ...
            {selectedTypes.length === 0 && (
              <div className="bf-desc-box" style={{background:'rgba(255,165,0,0.08)', color:'#E65C00', border:'1px solid rgba(230,92,0,0.3)'}}>⬆️ Pehle <strong>SP / DP / TP</strong> select karo</div>
            )}
            ...
            {needsDigit && spDpDigit === null && (
              <div className="bf-desc-box" style={{background:'rgba(255,165,0,0.08)', color:'#E65C00', border:'1px solid rgba(230,92,0,0.3)'}}>⬆️ Digit select karo</div>
            )}

            {allPanas.length > 0 && <>
              <div className="bf-desc-box" style={{background:'rgba(255,107,0,0.08)', color:'#E65C00', border:'1px solid rgba(230,92,0,0.4)'}}>
        ...
        {(id === 'sp_common' || id === 'dp_common') && <>
          ...
          {spDpDigit === null && (<div className="bf-desc-box" style={{ background:'rgba(255,165,0,0.08)', color:'#E65C00', border:'1px solid rgba(230,92,0,0.3)' }}>⬆️ Pehle digit select karo</div>)}
          {spDpDigit !== null && (() => {
            ...
            return <>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8, gap:8 }}>
                <div className="bf-desc-box" style={{ background:'rgba(255,107,0,0.08)', color:'#E65C00', border:'1px solid rgba(230,92,0,0.4)', margin:0, flex:1 }}>
                  🎯 Digit <strong>{spDpDigit}</strong> — <strong>{selectedCommon.length}/{allPanas.length}</strong> Panas selected
                </div>
                <div onClick={toggleAllCommon} style={{ padding:'8px 12px', background: selectedCommon.length === allPanas.length ? '#e53935' : '#FF6B00', color:'#fff', borderRadius:8, fontWeight:700, fontSize:12, cursor:'pointer', whiteSpace:'nowrap', flexShrink:0 }}>
              ...
              {selectedCommon.length === 0 && (<div className="bf-desc-box" style={{ background:'rgba(255,165,0,0.08)', color:'#E65C00', border:'1px solid rgba(230,92,0,0.3)' }}>⚠️ Kam se kam <strong>1 pana</strong> select karo</div>)}
        ...
        {id === 'cycle_panna' && <>
          ...
          {spDpDigit !== null && CYCLE_PANNA_DATA[spDpDigit] && <>
            <div className="bf-desc-box" style={{ background: 'rgba(255,107,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.4)' }}>🎯 <strong>Jodi {spDpDigit}</strong> — <strong>{CYCLE_PANNA_DATA[spDpDigit].length} Panas</strong> — Saare auto select honge</div>
            {(() => {
              ...
              return <>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8, gap:8 }}>
                  ...
                  <div onClick={toggleAllCycle} style={{ padding:'8px 12px', background: selectedCycle.length === allCyclePanas.length ? '#e53935' : '#FF6B00', color:'#fff', borderRadius:8, fontWeight:700, fontSize:12, cursor:'pointer', whiteSpace:'nowrap', flexShrink:0 }}>
                ...
                {selectedCycle.length === 0 && (<div className="bf-desc-box" style={{ background:'rgba(255,165,0,0.08)', color:'#E65C00', border:'1px solid rgba(230,92,0,0.3)' }}>⚠️ Kam se kam <strong>1 pana</strong> select karo</div>)}
          ...
          {spDpDigit === null && (<div className="bf-desc-box" style={{ background: 'rgba(255,165,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)' }}>⬆️ Pehle <strong>Jodi</strong> select karo</div>)}
        </>}
        ...
        {id === 'crossing_jodi' && (() => {
          ...
          return <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: 'rgba(255,107,0,0.05)', border: '1px solid rgba(230,92,0,0.3)', borderRadius: 10, marginBottom: 14, cursor: 'pointer' }} onClick={() => setCycleDigit(cycleDigit === 'cut' ? null : 'cut')}>
              <div style={{ width: 22, height: 22, borderRadius: 6, border: '2px solid #FF6B00', background: cycleDigit === 'cut' ? '#FF6B00' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s' }}>{cycleDigit === 'cut' && <span style={{ color: '#fff', fontWeight: 900, fontSize: 14 }}>✓</span>}</div>
              <span style={{ color: '#1a1a1a', fontWeight: 700, fontSize: 14 }}>Double Jodi Hatao (00, 11, 22...)</span>
            </div>
            {(!num || !num2) && (<div className="bf-desc-box" style={{ background: 'rgba(255,165,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)' }}>⬆️ Dono digit fields bharo</div>)}
            {num && num2 && generated.length === 0 && (<div className="bf-desc-box" style={{ background: 'rgba(255,165,0,0.08)', color: '#E65C00', border: '1px solid rgba(230,92,0,0.3)' }}>⚠️ Koi jodi nahi bani — digits check karo</div>)}
            {generated.length > 0 && <>
      ...
      <style>{`
        /* ============================================================
           MARUTI MATKA THEME — Orange / Cream / Dark Black
           Palette:
        ...
        /* ── BANNER ── */
        .bf-banner {
          background: linear-gradient(135deg, #1A0A00 0%, #2D1200 60%, #3D1800 100%);
          padding: 14px 16px;
          display: flex; align-items: center; gap: 12px;
          border-bottom: 3px solid #FF6B00;
          box-shadow: 0 3px 14px rgba(255,107,0,0.25);
        }
        .bf-banner-icon {
        ...
        .bf-banner-sub {
          font-size: 12px; color: rgba(255,200,150,0.8); margin-top: 2px;
        }
        ...
        .bf-card {
          ...
          border-radius: 16px;
          padding: 18px 16px;
          box-shadow: 0 4px 20px rgba(255,107,0,0.12), 0 1px 4px rgba(0,0,0,0.08);
        ...
        .bf-desc-box strong { color: #E65C00; }
        ...
        .bf-session-notice {
          ...
          padding: 8px 12px;
          margin-bottom: 10px;
          font-size: 12px; color: #E65C00; font-weight: 600; line-height: 1.5;
        }
        .bf-session-notice strong { color: #1A0A00; }

        /* ── FORM GROUP ── */
        .bf-fg   { margin-bottom: 14px; }
        .bf-label {
          font-size: 11px; color: #7A4A20; font-weight: 700;
          text-transform: uppercase; letter-spacing: 2px;
        ...
        .bf-input:focus { border-color: #FF6B00; box-shadow: 0 0 0 3px rgba(255,107,0,0.15); }
        .bf-input::placeholder { color: #C49A6C; }
        ...
        .bf-chip {
          ...
          font-size: 13px; cursor: pointer; font-weight: 700; color: #E65C00;
          transition: all 0.15s;
        }
        .bf-chip:hover { background: #FFE8CC; border-color: #FF6B00; }
        .bf-chip.active { background: #FF6B00; color: #fff; border-color: #FF6B00; box-shadow: 0 2px 8px rgba(255,107,0,0.3); }
        ...
        .bf-infobox {
          ...
          font-size: 13px; color: #7A4A20;
        }
        .bf-infobox strong { color: #E65C00; font-size: 14px; }
        ...
        .bf-nchip {
          ...
          cursor: pointer; color: #1A0A00; transition: all 0.15s;
        }
        .bf-nchip:hover { border-color: #FF6B00; color: #FF6B00; background: #FFF0E0; }
        .bf-nchip.active {
          background: #FF6B00;
          color: #fff;
          border-color: #FF6B00;
        ...
        .bf-jodi-scroll {
          ...
          border: 1.5px solid #FFD4A8; border-radius: 10px;
          background: #FFF8F2; margin-bottom: 4px;
        }
        .bf-jodi-scroll::-webkit-scrollbar { width: 4px; }
        .bf-jodi-scroll::-webkit-scrollbar-thumb { background: #FFB870; border-radius: 4px; }
        .bf-jodi-grid { display: grid; grid-template-columns: repeat(5,1fr); }
        .bf-jchip {
          padding: 10px 4px; text-align: center;
          font-size: 12px; font-weight: 700; cursor: pointer; color: #5A3010;
          border-right: 1px solid #FFE8CC; border-bottom: 1px solid #FFE8CC;
          transition: all 0.12s;
        }
        .bf-jchip:hover { background: #FFF0E0; color: #FF6B00; }
        .bf-jchip.active { background: #FF6B00; color: #fff; }
        ...
        .bf-pana-grid::-webkit-scrollbar { width: 4px; }
        .bf-pana-grid::-webkit-scrollbar-thumb { background: #FFB870; border-radius: 4px; }
        .bf-pchip {
          background: #FFF8F2;
          border: 1.5px solid #FFD4A8;
          border-radius: 8px; padding: 8px 4px;
          text-align: center; font-size: 12px; font-weight: 700;
          cursor: pointer; color: #5A3010; transition: all 0.12s;
        }
        .bf-pchip:hover { border-color: #FF6B00; color: #FF6B00; background: #FFF0E0; }
        .bf-pchip.active { background: #FF6B00; color: #fff; border-color: #FF6B00; }
        ...
        .bf-session-btn:hover { border-color: #FF6B00; color: #FF6B00; }
        .bf-session-btn.active {
          background: #FF6B00;
          color: #fff;
          border-color: #FF6B00;
        ...
        .bf-session-btn.disabled:hover { border-color: #FFD4A8; color: #7A4A20; }
        ...
        .bf-add-btn:hover { background: #FFE8CC; }
        ...
        .bf-table th {
          background: #1A0A00;
          color: #FF6B00; padding: 9px 10px; text-align: left;
          font-weight: 700; letter-spacing: 1px; font-size: 11px;
          font-family: 'Teko', sans-serif;
        }
        .bf-table td { padding: 9px 10px; border-bottom: 1px solid #FFE8CC; color: #3A1800; }
        .bf-table tr:last-child td { border-bottom: none; }
        .bf-del {
          background: rgba(229,57,53,0.1); color: #e53935;
          border: 1px solid rgba(229,57,53,0.3); border-radius: 5px;
          padding: 3px 8px; font-size: 11px; cursor: pointer; font-weight: 700; transition: all 0.15s;
        }
        .bf-del:hover { background: #e53935; color: #fff; }
        ...
        .bf-total-row span   { font-size: 13px; color: #7A4A20; font-weight: 600; }
        .bf-total-row strong { font-size: 16px; color: #E65C00; font-weight: 800; }

        /* ── PLACE BET BUTTON — matches app's rounded orange button ── */
        .bf-place-btn {
          width: 100%;
          background: #FF6B00;
          color: #FFFFFF;
        ...
        .bf-place-btn:hover:not(:disabled) {
