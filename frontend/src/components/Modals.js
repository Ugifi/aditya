...
// ────────────────────────────────────────────────
//  ADD MONEY MODAL — Maruti Matka Orange Theme
// ────────────────────────────────────────────────
export function AddModal({ onClose, onSuccess }) {
  ...
  // ── Maruti Matka Orange Theme Tokens ──
  ...
  const S = {
    ...
    modal: {
      ...
      padding: '24px',
      boxShadow: '0 -10px 40px rgba(249,115,22,0.18)',
      maxHeight: '90vh', overflowY: 'auto',
      animation: 'slideUp 0.3s ease-out',
      border: '1px solid rgba(249,115,22,0.20)',
      borderTop: '3px solid #F97316',
    },
    handle: { width: 40, height: 4, background: 'rgba(249,115,22,0.4)', borderRadius: 4, margin: '0 auto 16px' },
    title: { margin: '0 0 20px', color: '#1C0A00', fontSize: 18, fontWeight: 900, letterSpacing: 1, textAlign: 'center' },
    fg: { marginBottom: 16 },
    label: { fontSize: 11, color: '#C2530A', fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6, display: 'block', marginLeft: 4 },
    input: {
      width: '100%', background: '#FFF7ED',
      border: '2px solid rgba(249,115,22,0.30)', borderRadius: 14,
      padding: '14px', color: '#1C0A00', fontSize: 15, fontWeight: 700,
      outline: 'none', boxSizing: 'border-box'
    },
    btnPrimary: {
      width: '100%',
      background: 'linear-gradient(90deg, #F97316, #C2530A)',
      color: '#FFFFFF', border: 'none', borderRadius: 14,
      padding: '16px', fontSize: 15, fontWeight: 900, cursor: 'pointer',
      letterSpacing: 1, textTransform: 'uppercase',
      boxShadow: '0 6px 20px rgba(249,115,22,0.35)', marginBottom: 12
    },
    btnSecondary: {
      width: '100%', background: 'rgba(255,23,68,0.06)', color: '#ff1744',
  ...
  return (
    <div style={S.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <style>{`
        ...
        .mk-input:focus { border-color: #F97316 !important; box-shadow: 0 0 0 3px rgba(249,115,22,0.15); }
        .mk-input::placeholder { color: rgba(28,10,0,0.35) !important; }
        .mk-chip-btn { transition: all 0.2s ease; }
        .mk-chip-btn:hover { transform: translateY(-1px); }
        option { background: #FFF7ED; color: #1C0A00; }
      `}</style>
      ...
      <div style={S.modal}>
        ...
        {step === 1 && <>
              ...
              {chips.map(c => (
                <button
                  ...
                  style={{
                    ...
                    background: chip === c ? 'linear-gradient(90deg, #F97316, #C2530A)' : '#FFF7ED',
                    color: chip === c ? '#FFFFFF' : '#F97316',
                    border: chip === c ? 'none' : '1.5px solid rgba(249,115,22,0.35)',
          ...
          {upiId && (
            <div style={{
              ...
              fontSize: 13, color: 'rgba(28,10,0,0.6)', textAlign: 'center', fontWeight: 700
            }}>
              💳 Pay to UPI: <strong style={{ color: '#F97316' }}>{upiId}</strong>
            </div>
          )}
        ...
        {step === 2 && <>
          ...
          <div style={{ textAlign: 'center', marginBottom: 16 }}>
            <div style={{
              ...
              display: 'inline-block', border: '2px solid rgba(249,115,22,0.30)'
            }}>
              {qrUrl
                ? <img src={qrUrl} alt="QR Code" width={180} height={180} style={{ display: 'block', borderRadius: 8 }} />
                : <div style={{ width: 180, height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(249,115,22,0.5)', fontSize: 13, fontWeight: 700 }}>Loading QR...</div>
              }
            </div>
            <div style={{ fontSize: 12, color: 'rgba(28,10,0,0.5)', marginTop: 12, fontWeight: 800 }}>
              UPI ID: <span style={{ color: '#F97316' }}>{upiId}</span>
            </div>
            <div style={{ fontSize: 24, fontWeight: 900, color: '#16a34a', marginTop: 6 }}>
          ...
          <div style={{
            ...
            fontSize: 12, color: 'rgba(28,10,0,0.6)', marginBottom: 16,
            lineHeight: 1.6, fontWeight: 600
          }}>
            ✅ Pay karke <strong style={{ color: '#F97316' }}>Transaction No / UTR</strong> daalo<br />
            ⏰ Credit within <strong style={{ color: '#F97316' }}>30 minutes</strong>
          </div>
          ...
          <button style={{ ...S.btnSecondary, color: '#F97316', background: '#FFF7ED', border: '2px solid rgba(249,115,22,0.25)' }} onClick={() => setStep(1)}>
            ← BACK
          </button>
        </>}
        ...
        {step === 3 && <>
          ...
          <div style={{
            ...
            fontSize: 14, color: 'rgba(28,10,0,0.7)', lineHeight: 1.6, textAlign: 'center'
          }}>
            <div style={{ fontSize: 40, marginBottom: 10 }}>✅</div>
            Deposit <strong style={{ fontSize: 16, color: '#16a34a' }}>₹{Number(amt).toLocaleString()}</strong> request submitted!<br />
            UTR: <strong style={{ color: '#F97316' }}>{txnNo}</strong><br /><br />
            Admin approve karega — <strong style={{ color: '#16a34a' }}>30 min</strong> mein wallet mein credit hoga.
          </div>
...
// ────────────────────────────────────────────────
//  WITHDRAW MODAL — Maruti Matka Orange Theme
// ────────────────────────────────────────────────
export function WithdrawModal({ wallet, onClose, onSuccess }) {
  ...
  const S = {
    ...
    input: {
      width: '100%', background: '#FFF7ED',
      border: '2px solid rgba(249,115,22,0.30)', borderRadius: 12,
      padding: '12px 14px', color: '#1C0A00', fontSize: 14, fontWeight: 700,
      outline: 'none', boxSizing: 'border-box'
    },
    btnPrimary: {
      width: '100%',
      ...
      <div style={S.modal}>
        ...
        {step === 1 ? <>
          ...
          <div style={{
            ...
            <span style={{ color: '#16a34a', fontSize: 13, fontWeight: 800 }}>
              💰 Available: ₹{wallet.toLocaleString()}
            </span>
            <span style={{ color: '#F97316', fontSize: 12, fontWeight: 800 }}>Min: ₹{MIN}</span>
          </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 4 }}>
              {[
                ...
                <button
                  ...
                  style={{
                    flex: 1, padding: '12px',
                    background: method === m.id ? 'linear-gradient(90deg, #F97316, #C2530A)' : '#FFF7ED',
                    color: method === m.id ? '#FFFFFF' : '#F97316',
                    border: method === m.id ? 'none' : '1.5px solid rgba(249,115,22,0.35)',
          ...
          <div style={{
            ...
            <div style={{ fontSize: 40, marginBottom: 10 }}>⏳</div>
            Withdrawal <strong style={{ fontSize: 16, color: '#F97316' }}>₹{Number(amt).toLocaleString()}</strong> request bheja gaya!<br />
            Method: <strong style={{ color: '#F97316' }}>{method === 'upi' ? `UPI — ${upiId}` : `Bank — ${bankName}`}</strong><br /><br />
            Processing Time: <strong style={{ color: '#C2530A' }}>0–5 hours</strong>
          </div>
          <button style={S.btnPrimary} onClick={() => { onSuccess(Number(amt)); onClose(); }}>DONE</button>
        </>}
      </div>
